...
def load_country_codes(filename: Path = COUNTRY_CODE_FILENAME) -> Dict[Dict[str, str]]:
	codes = dict()
	with filename.open('r') as code_file:
		...
		for line in reader:
			...
			codes[name] = asdict(CountryCode(name, iso2, iso3, m49))  # Use a dict so fuzzy search is easier.
	return codes
